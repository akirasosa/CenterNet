...
from models.networks import efficientnet_centernet, msra_resnet, mobilenet_centernet
...
# %%
# torch.onnx.export(net, x, Path.home() / 'tmp' / 'mobilenet_centernet.onnx', verbose=True)
# torch.save(net.state_dict(), Path.home() / 'tmp' / 'mobilenet_dcn_no-head_torch.pth')
torch.save(net_trt.state_dict(), Path.home() / 'tmp' / 'mobilenet_centernet_trt.pth')
#
# net = net.half()
# x = x.half()
